package com.mall.dayi.ui.widget;
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    private static final int HEADER_VIEW_HEIGHT = 40;// HeaderView height (dp)
    ...
    private static int DEFAULT_CIRCLE_TARGET = 60;
    ...
    public SuperSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.refresh_pull_push);
        int footerHeight = ta.getInteger(R.styleable.refresh_pull_push_footerHeight,
                DEFAULT_CIRCLE_TARGET);
        int headerHeight = ta.getInteger(R.styleable.refresh_pull_push_headerHeight,
                DEFAULT_CIRCLE_TARGET);
        isShowFooter = ta.getBoolean(R.styleable.refresh_pull_push_isShowFooter, false);
        ta.recycle();
        DEFAULT_CIRCLE_TARGET = headerHeight;
        /**
         * getScaledTouchSlop是一个距离，表示滑动的时候，手的移动要大于这个距离才开始移动控件。如果小于这个距离就不触发移动控件
         */
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();

        mMediumAnimationDuration = getResources().getInteger(android.R.integer
    ...
    private void createHeaderViewContainer() {
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams((int)
                (mHeaderViewHeight * 0.8), (int) (mHeaderViewHeight * 0.8));
    ...
    public void setRefreshing(boolean refreshing) {
        if (refreshing && mRefreshing != refreshing) {
            ...
            setTargetOffsetTopAndBottom(endTarget - mCurrentTargetOffsetTop, true /* requires
            update */);
            mNotify = false;
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mTarget.measure(MeasureSpec.makeMeasureSpec(getMeasuredWidth() - getPaddingLeft() -
                getPaddingRight(), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec
                (getMeasuredHeight() - getPaddingTop() - getPaddingBottom(), MeasureSpec.EXACTLY));
        mHeadViewContainer.measure(MeasureSpec.makeMeasureSpec(mHeaderViewWidth, MeasureSpec
                .EXACTLY), MeasureSpec.makeMeasureSpec(3 * mHeaderViewHeight, MeasureSpec.EXACTLY));
        mFooterViewContainer.measure(MeasureSpec.makeMeasureSpec(mFooterViewWidth, MeasureSpec
                .EXACTLY), MeasureSpec.makeMeasureSpec(mFooterViewHeight, MeasureSpec.EXACTLY));
    ...
    public boolean isChildScrollToTop() {
        if (Build.VERSION.SDK_INT < 14) {
            if (mTarget instanceof AbsListView) {
                ...
                return !(absListView.getChildCount() > 0 && (absListView.getFirstVisiblePosition
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || mRefreshing || mLoadMore || (!isChildScrollToTop
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mPrevX = ev.getX();
                setTargetOffsetTopAndBottom(mOriginalOffsetTop - mHeadViewContainer.getTop(),
                        true);// 恢复HeaderView的初始位置
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    float tensionSlingshotPercent = Math.max(0, Math.min(extraOS, slingshotDist *
                            2) / slingshotDist);
    ...
    public void resetTargetLayout() {
        ...
        int headViewWidth = mHeadViewContainer.getMeasuredWidth();
        int headViewHeight = mHeadViewContainer.getMeasuredHeight();
        mHeadViewContainer.layout((width / 2 - headViewWidth / 2), -headViewHeight, (width / 2 +
                headViewWidth / 2), 0);// 更新头布局的位置
        int footViewWidth = mFooterViewContainer.getMeasuredWidth();
        int footViewHeight = mFooterViewContainer.getMeasuredHeight();
        mFooterViewContainer.layout((width / 2 - footViewWidth / 2), height, (width / 2 +
                footViewWidth / 2), height + footViewHeight);
    }
